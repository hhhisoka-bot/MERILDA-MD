...
const handler = async (m, { conn, args }) => {
  ...
  if (!value) {
    ...
    return m.reply(`🔧 *PARAMÈTRE ANTI-VIEWONCE*\n\n📝 *Statut:* ${status}\n\n*Usage:* ${globalThis.prefix?.main || '.'}antiviewonceset on/off`)
  }
  ...
  db.updateGroupSetting(m.chat, 'antiviewonce', newValue)
  m.reply(`✅ Anti-ViewOnce ${newValue ? 'activé' : 'désactivé'} avec succès !`)
}

handler.help = ['antiviewonceset']
handler.tags = ['group']
handler.command = ['antiviewonceset', 'antiviewoncesetting']
handler.group = true
handler.admin = true
