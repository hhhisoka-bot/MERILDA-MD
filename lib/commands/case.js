...
if (time2 < "03:00:00") {
  ...
  ucapangreet = "Bonjour très tôt 🌆"
} else if (time2 < "11:00:00") {
  ucapangreet = "Bonjour 🏙️"
} else if (time2 < "15:00:00") {
  ucapangreet = "Bon après-midi 🏞️"
...
const wib = moment(Date.now()).tz("Africa/Abidjan").locale("id").format("HH:mm:ss z")
const wita = moment(Date.now()).tz("Africa/Dakar").locale("id").format("HH:mm:ss z")
const wit = moment(Date.now()).tz("Africa/Yaounde").locale("id").format("HH:mm:ss z")
...
export default async (conn, m, chatUpdate, store) => {
  try {
    ...
    const fcontact = {
      ...
      message: {
        contactMessage: {
          displayName: global.botName || "MERILDA-MD ",
          vcard: `BEGIN:VCARD
VERSION:3.0
N:XL;${pushname},;;;
FN:${pushname}
