...
async function sendWelcomeMessage(whatsappConnection, userJid) {
  try {
    // Vérifier si config.bot existe, sinon utiliser des valeurs par défaut
    const botName = config.bot?.name || config.botName || "RAV Bot"
    const botAuthor = config.bot?.author || config.owner?.name || "RAV Team"
    const prefix = config.PREFIX || "."
    
    const welcomeMessage = `*🎉 Connexion réussie !*

*『${botName}』 est maintenant en ligne ! 🚀*
...
> © Powered by ${botAuthor}`

    // Envoyer le message avec l'image
    await whatsappConnection.sendMessage(userJid, {
      image: { url: `https://files.catbox.moe/4c8ql3.jpg` },
      caption: welcomeMessage,
    })
    
    console.log(chalk.green(`✅ Message de bienvenue envoyé à ${userJid}`))
  } catch (error) {
    console.error(chalk.red(`❌ Erreur lors de l'envoi du message de bienvenue: ${error.message}`))
  }
}
...
let isNewSession = false // Nouvelle variable pour tracker si c'est une nouvelle session
...
async function startWhatsAppBot() {
  ...
  const sessionInfo = await loadSessionFromSources()
  ...
  // Marquer comme nouvelle session si aucune session n'existe
  if (!sessionInfo.exists) {
    isNewSession = true
  }
  ...
  whatsappConnection.ev.on("connection.update", async (update) => {
    ...
    if (connection === "open") {
      ...
      if (sessionInfo.exists && !isNewSession) {
      ...
      // Obtenir le JID de l'utilisateur
      ...
      if (isNewSession) {
        ...
        // Envoyer le message de bienvenue pour une nouvelle session
        setTimeout(async () => {
          await sendWelcomeMessage(whatsappConnection, userJid)
        }, 3000) // Attendre 3 secondes pour s'assurer que la connexion est stable
      }

      // Envoi d'un message de confirmation au propriétaire (si différent de l'utilisateur)
      if (config.creator && config.creator !== userJid) {
